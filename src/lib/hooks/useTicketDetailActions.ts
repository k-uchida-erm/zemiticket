...
export function useTicketDetailActions(params: UseTicketDetailActionsParams) {
	...
	const toggleTodo = useCallback(
		(subIdx: string, todoId: string) => {
			return async () => {
				try {
					const sub = subs.find(s => s.id === subIdx);
					if (!sub) return;
					...
					const response = await fetch(`/api/todos/update`, {
						...
						body: JSON.stringify({ todoId, done: newDoneState }),
					});
						...
						prev.map(s =>
							s.id === subIdx
								? {
										...s,
										todos: (s.todos || []).map(t =>
											t.id === todoId ? { ...t, done: newDoneState } : t
										),
									}
								: s
						)
					...
					window.dispatchEvent(
						new CustomEvent('todoProgressUpdated', {
	...
	const startEditingSub = useCallback(
		(subIdx: string) => {
			...
			const todoEstimates: Record<string, string> = {};
			sub.todos?.forEach(todo => {
				todoTitles[todo.id] = todo.title;
				todoEstimates[todo.id] = (todo.estimateHours ?? '').toString();
			});
	...
	const saveEditingSub = useCallback(
		async (subIdx: string) => {
			const sub = subs.find(s => s.id === subIdx);
			if (!sub) return;
			...
			try {
				...
				const todoUpdatePromises = (sub.todos || []).map(async todo => {
					...
					const parsedEstimate =
						newEstimateRaw === undefined
							? undefined
							: newEstimateRaw === ''
								? 0
								: Number(newEstimateRaw);
					const currentEstimate = todo.estimateHours ?? 0;
					const shouldUpdateTitle =
						newTodoTitle !== undefined && newTodoTitle.trim() !== todo.title;
					const shouldUpdateEstimate =
						parsedEstimate !== undefined && parsedEstimate !== currentEstimate;
					if (shouldUpdateTitle || shouldUpdateEstimate) {
						const body: { title?: string; estimate_hours?: number } = {};
						...
						s.id === subIdx
							? {
									...
									todos: (s.todos || []).map(todo => {
										const hasNewEstimate = Object.prototype.hasOwnProperty.call(
											todoEstimates,
											todo.id
										);
										const nextEstimate = hasNewEstimate
											? todoEstimates[todo.id] === ''
												? 0
												: Number(todoEstimates[todo.id])
											: todo.estimateHours;
										return {
											...todo,
											title:
												todoTitles[todo.id] !== undefined &&
												todoTitles[todo.id].trim() !== ''
													? todoTitles[todo.id]
													: todo.title,
	...
	const cancelEditingSub = useCallback(
		(subIdx: string) => {
			const sub = subs.find(s => s.id === subIdx);
			if (!sub) return;
			setEditingSub(prev => ({ ...prev, [sub.id]: false }));
	...
	const reorderSubtasks = useCallback(
		async (newOrder: (SubTask & { todos?: SubTodo[] })[]) => {
			try {
				setSubs(newOrder);
				const updatePromises = newOrder.map((subtask, index) =>
					fetch(`/api/sub-tasks/${subtask.id}/update`, {
						method: 'PUT',
						headers: { 'Content-Type': 'application/json' },
	...
	const reorderTodos = useCallback(
		async (subtaskId: string, newOrder: SubTodo[]) => {
			try {
				setSubs(prev =>
					prev.map(s => (s.id === subtaskId ? { ...s, todos: newOrder } : s))
				);
	...
	const createSub = useCallback(async () => {
		...
		try {
			...
			setSubs(prev => [
				...prev,
				{ ...newSubTask, user: parent.user, description: '', todos: [] },
			]);
			if (setOpenTodos) setOpenTodos(p => ({ ...p, [newSubTask.id]: true }));
			if (setAddingSub) setAddingSub(false);
	...
	const addTodo = useCallback(
		async (subIdx: string, subId: string) => {
			const title = ((newTodoTitle && newTodoTitle[subId]) || '').trim();
			if (!title) return;
			const estimateHours =
				newTodoEstimate && newTodoEstimate[subId]
					? newTodoEstimate[subId] === ''
						? 0
						: Number(newTodoEstimate[subId])
					: 0;
			try {
				...
				setSubs(prev =>
					prev.map(s =>
						s.id === subIdx ? { ...s, todos: [...(s.todos || []), mapped] } : s
					)
				);
				if (setAddingTodo) setAddingTodo(p => ({ ...p, [subId]: false }));
				if (setNewTodoTitle) setNewTodoTitle(p => ({ ...p, [subId]: '' }));
