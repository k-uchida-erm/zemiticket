...
export function useTicketDetailActions(params: UseTicketDetailActionsParams) {
	...
	const toggleTodo = useCallback((subIdx: string, todoId: string) => {
		return async () => {
			try {
				...
				setSubs((prev) => prev.map((s) => s.id === subIdx ? { ...s, todos: (s.todos || []).map((t) => t.id === todoId ? { ...t, done: newDoneState } : t) } : s));
	...
	const startEditingSub = useCallback((subIdx: string) => {
		const sub = subs.find(s => s.id === subIdx);
		if (!sub) return;
	...
	const saveEditingSub = useCallback(async (subIdx: string) => {
		const sub = subs.find(s => s.id === subIdx);
		if (!sub) return;
		...
		try {
			...
			setSubs((prev) => prev.map((s) => s.id === subIdx ? {
				...s,
				title: newTitle.trim(),
	...
	}, [editingSubTitle, editingTodoTitles, editingTodoEstimates, setSubs, setEditingSub, setEditingSubTitle, setEditingTodoTitles, setEditingTodoEstimates, parent.id]);

	const cancelEditingSub = useCallback((subIdx: string) => {
		const sub = subs.find(s => s.id === subIdx);
		if (!sub) return;
		setEditingSub((prev) => ({ ...prev, [sub.id]: false }));
		setEditingSubTitle((prev) => ({ ...prev, [sub.id]: '' }));
		setEditingTodoTitles((prev) => ({ ...prev, [sub.id]: {} }));
		setEditingTodoEstimates((prev) => ({ ...prev, [sub.id]: {} }));
	}, [setEditingSub, setEditingSubTitle, setEditingTodoTitles, setEditingTodoEstimates]);

	const deleteTodo = useCallback((subIdx: string, todoId: string) => {
		return async () => {
			try {
				const response = await fetch(`/api/todos/${todoId}/delete`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' } });
				if (!response.ok) throw new Error('Failed to delete todo');
				setSubs((prev) => prev.map((s) => s.id === subIdx ? { ...s, todos: (s.todos || []).filter((t) => t.id !== todoId) } : s));
				window.dispatchEvent(new CustomEvent('todoProgressUpdated', { detail: { parentTaskId: parent.id } }));
	...
	const reorderTodos = useCallback(async (subtaskId: string, newOrder: SubTodo[]) => {
		try {
	...
	const addTodo = useCallback(async (subIdx: string, subId: string) => {
		...
		try {
			...
			setSubs((prev) => prev.map((s) => s.id === subIdx ? { ...s, todos: [...(s.todos || []), mapped] } : s));
			if (setAddingTodo) setAddingTodo((p) => ({ ...p, [subId]: false }));
			if (setNewTodoTitle) setNewTodoTitle((p) => ({ ...p, [subId]: '' }));
			if (setNewTodoEstimate) setNewTodoEstimate((p) => ({ ...p, [subId]: '' }));
