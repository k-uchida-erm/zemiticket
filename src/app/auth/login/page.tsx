			...
			if (access_token && refresh_token) {
				(async () => {
					...
					if (typeof window !== 'undefined')
						window.location.replace(`${baseUrl}${dest}`);
		...
		<div className='min-h-screen grid place-items-center bg-white'>
			<div className='w-full max-w-sm border border-neutral-200 bg-white p-5'>
				<div className='flex items-center gap-2 mb-2'>
					<span className='inline-flex w-7 h-7 items-center justify-center rounded-xl bg-[#00b393]/10 border border-[#00b393]/20 text-[#00b393] font-semibold'>
						Z
					</span>
				...
				{userEmail ? (
					<div className='mt-2 text-sm text-neutral-800 space-y-2'>
						...
						<button
							onClick={onSignOut}
							className='text-sm px-3 py-1.5 border border-neutral-300'
						>
							Sign out
						</button>
					</div>
					...
					<>
						<div className='mt-2 grid grid-cols-2 gap-2'>
							<button
								...
								disabled={loading}
								className='text-sm px-3 py-2 border border-neutral-300 hover:bg-neutral-50'
							>
						...
						{sent ? (
							...
							<form onSubmit={onSubmit} className='space-y-3'>
								...
								<button
									type='submit'
									disabled={loading}
									className='w-full px-3 py-2 text-sm border border-neutral-300 bg-neutral-900 text-white disabled:opacity-60'
								>
									Send magic link
								</button>
