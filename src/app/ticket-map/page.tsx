...
export default function TicketMap() {
	const [_ticketsData, setTicketsData] = useState<TicketGroup[]>([]);
				...
				if (response.ok) {
					...
					if (result.data) {
						...
						result.data.forEach((epicGroup: TicketGroup) => {
							epicGroup.list?.forEach(
								(item: { parent: ParentTask; children: SubTask[] }) => {
									if (item.parent) {
	...
	const handleDrop = (e: React.DragEvent, targetStatus: string) => {
		...
		if (draggedTicket && draggedTicket.fromStatus !== targetStatus) {
			...
			setTicketsByStatus(prev => {
				...
				// 元のステータスから削除
				newState[draggedTicket.fromStatus as keyof typeof prev] = prev[
					draggedTicket.fromStatus as keyof typeof prev
				].filter((ticket: ParentTask) => ticket.id !== draggedTicket.id);

				// 新しいステータスに追加
				const ticketToMove = prev[
					draggedTicket.fromStatus as keyof typeof prev
				].find((ticket: ParentTask) => ticket.id === draggedTicket.id);

				if (ticketToMove) {
					newState[targetStatus as keyof typeof prev] = [
						...prev[targetStatus as keyof typeof prev],
						ticketToMove,
					];
				}
	...
	return (
		<div className='min-h-screen bg-white p-6'>
			<SectionTitle icon={<Ticket />}>Ticket Map</SectionTitle>
					...
					{Object.entries(statusConfig).map(([status, config]) => (
						...
						>
							...
							<div className='space-y-2 pl-0.5 pr-0.5'>
								{ticketsByStatus[status as keyof typeof ticketsByStatus].map(
									(ticket: ParentTask) => (
										<div
											key={ticket.id}
											...
											draggable
											onDragStart={e =>
												handleDragStart(e, ticket.id.toString(), status)
											}
											style={{
												opacity:
													draggedTicket?.id === ticket.id.toString() ? 0.5 : 1,
											}}
										>
											{/* エピックラベル */}
											{ticket.epic && (
