...
interface TicketDetailPanelProps {
	...
	subtasks?: SubTask[];
	onClose: () => void;
	_onSave?: (data: { title: string; description: string; due: string }) => void;
...
export default function TicketDetailPanel({
	...
	const { reorderSubtasks, reorderTodos } = useTicketDetailReorder();

	const { startEditingSub, cancelEditingSub } = useTicketDetailEditing();
	...
	return (
		<div className='pt-5 pb-20 relative'>
			...
			<SortableSubTasksContainer
				...
				subtasks={subs}
				openTodos={openTodos}
				onToggleTodos={(subtaskId: string) =>
					setOpenTodos(prev => ({ ...prev, [subtaskId]: !prev[subtaskId] }))
				}
				onToggleTodo={(subtaskId: string, todoId: string) => {
					...
					if (subIdx !== -1) {
						...
						if (todoIdx !== undefined && todoIdx !== -1 && updatedSub.todos) {
							updatedSub.todos[todoIdx] = {
								...updatedSub.todos[todoIdx],
								done: !updatedSub.todos[todoIdx].done,
							};
							updatedSubs[subIdx] = updatedSub;

							// 進捗を計算
							const totalTodos = updatedSubs.reduce(
								(total, sub) => total + (sub.todos?.length || 0),
								0
							);
							const completedTodos = updatedSubs.reduce(
								(total, sub) =>
									total + (sub.todos?.filter(t => t.done).length || 0),
								0
							);
							const progress =
								totalTodos > 0
				...
				onReorderSubtasks={newOrder =>
					reorderSubtasks(newOrder, subtasks, setSubs)
				}
				onReorderTodos={(subtaskId, newOrder) =>
					reorderTodos(subtaskId, newOrder, subtasks, setSubs)
				}
