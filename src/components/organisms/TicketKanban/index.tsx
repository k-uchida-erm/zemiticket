...
export default function TicketKanban({
	...
	const activeParentTickets: KanbanTask[] = useMemo(
	...
	const [expandedSubtasks, setExpandedSubtasks] = useState<
		Set<string | number>
	>(new Set());
	const [manuallyUpdatedSubtasks, setManuallyUpdatedSubtasks] = useState<
		Set<string | number>
	>(new Set());
	...
	const onDropActiveParent = useCallback(
		(e: React.DragEvent) => {
			e.preventDefault();
	...
	const onDropActiveSub = useCallback(
		(e: React.DragEvent, targetStatus: 'todo' | 'active' | 'completed') => {
			e.preventDefault();
			const raw = e.dataTransfer.getData('application/json');
			if (!raw) return;
			try {
	...
	const _onDropCompletedSub = useCallback(
		(e: React.DragEvent) => {
			e.preventDefault();
			const raw = e.dataTransfer.getData('application/json');
			if (!raw) return;
	...
	const handleTodoToggle = useCallback(
		async (subtaskId: string, todoId: string, done: boolean) => {
			try {
									...
									ticket.subtasks ? (
										<div className='mt-6'>
											...
											<div className='absolute top-4 right-8 z-10 flex gap-2'>
												...
												<DeactivateButton
													onDeactivate={() =>
														onTicketActivate?.(ticket.id.toString(), false)
													}
												/>
											</div>
