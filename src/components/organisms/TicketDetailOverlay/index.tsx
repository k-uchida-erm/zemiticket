...
interface TicketDetailOverlayProps {
	...
	onClose: () => void;
	ticketsData: Array<{
		list: Array<{ parent: ParentTask; children: ParentTask[] }>;
	}>;
}
...
export default function TicketDetailOverlay({
	...
	return (
		<div
			className={`absolute inset-0 h-full w-full bg-white transition-transform duration-300 translate-x-full ${selected || creatingEpic ? '!translate-x-0' : ''} z-50 overflow-hidden ${selected || creatingEpic ? 'border-l border-neutral-200' : ''}`}
		>
			<div
			...
			>
				{creatingEpic ? (
					<TicketCreateForm
						defaultEpic={creatingEpic}
						onCancel={onClose}
						onCreate={() => onClose()}
					/>
				) : selected ? (
					(() => {
						const group = ticketsData
							.flatMap(g => g.list)
							.find(g => g.parent.id === selected.id);
						return (
							<TicketDetailPanel
								parent={selected}
