...
export default function SubmittedTicketCard({
	ticket,
}: SubmittedTicketCardProps) {
	const parent = toParentTask(ticket);
	const children: SubTask[] = ticket.children || [];
	const latestComment =
		Array.isArray(ticket.reviewComments) && ticket.reviewComments.length > 0
	...
	const badge = useMemo(() => {
		const isPending =
			ticket.reviewDecision === 'pending' || !ticket.reviewDecision;
		...
		return (
			<span
				className={`shrink-0 text-[10px] px-2 py-[2px] rounded-full ${cls}`}
			>
				{label}
			</span>
		);
