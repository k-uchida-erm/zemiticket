...
interface ParentTicketCardProps {
	...
	expanded?: boolean;
	onToggle?: () => void;
	_onToggleTodo?: (subtaskId: number, todoId: number) => void;
	size?: 'md' | 'sm' | 'xs';
...
export default function ParentTicketCard({
	parent,
	subtasks,
	children,
	expanded = true,
...
}: ParentTicketCardProps) {
	const subtasksList: SubTask[] = Array.isArray(subtasks)
		? subtasks
		: children || [];

	// 進捗の初期値を計算（データベースの値がない場合や0の場合は計算で求める）
	const initialProgress =
		parent.progressPercentage && parent.progressPercentage > 0
			? parent.progressPercentage
			: calcProgress(subtasksList);
	...
	useEffect(() => {
				...
				if (isRelatedTodo) {
					...
					try {
						const response = await fetch(
							`/api/parent-tasks/${parent.id}/progress`
						);
						if (response.ok) {
		...
		window.addEventListener(
			'todoProgressUpdated',
			handleProgressUpdate as unknown as EventListener
		);
	...
	const willWrapWholeCard =
		!renderSubticketsInside && !!parent.slug && !disableLinks;

	const titleNode = willWrapWholeCard ? (
		<>{parent.title}</>
	) : parent.slug && !disableLinks ? (
		<Link
			href={`/ticket/${parent.slug}`}
			aria-label={`${parent.title} details`}
			className='focus:outline-none focus:ring-2 focus:ring-emerald-400 rounded-none'
		>
			{parent.title}
		</Link>
	) : (
	...
	const body = (
		<Card
			className={`${cardPadding} ${minHeight} relative shadow-sm rounded-md`}
		>
			{parent.due && !hideDue && (
				<div className={`absolute top-0 right-10 z-10 flex items-center gap-1`}>
					<div className='rounded-t-none rounded-b-lg bg-white font-medium text-[14px] pl-2.5 pr-3 py-1.5 tracking-[0.02em] leading-none tabular-nums border-x border-b border-neutral-200/70 text-center'>
						<span className="text-neutral-700 font-['Hiragino Kaku Gothic ProN','Noto Sans JP',sans-serif]">
							due {formatDue(parent.due)}
						</span>
					</div>
					<span
						className={`inline-flex items-center justify-center [&_svg]:w-5 [&_svg]:h-5 ${mood === 'great' ? 'text-red-500' : mood === 'rush' ? 'text-blue-500' : 'text-amber-500'}`}
					>
						{mood === 'great' ? (
							<IconMoodGreat />
						) : mood === 'rush' ? (
			...
			<div
				className={`flex items-start justify-between gap-4 ${contentTopPad}`}
			>
				<div className='flex-1 min-w-0'>
					...
					<div
						className={`flex items-start justify-between ${titleRowGap} ${titleRowMt}`}
					>
						<div className='min-w-0 flex items-center gap-2'>
							...
							{showToggle && (
								<button
									...
									aria-label={
										expanded ? 'Collapse subtickets' : 'Expand subtickets'
									}
					...
					{parent.description && (
						<p
							className={`${descMt} ${descText} leading-5 text-neutral-600 line-clamp-2`}
						>
							{parent.description}
						</p>
					)}
					...
					{!hideProgress && (
						<div className={progressMt}>
							...
							<div
								className={`mt-1.5 flex items-center gap-3 ${metaText} text-neutral-600`}
							>
								...
								<>
									<ul className='space-y-0 divide-y divide-neutral-200 border-t border-neutral-200 mt-2'>
										{subtasksList.map((child, index) => (
											<li
												key={child.id}
												className={`relative ${index === subtasksList.length - 1 ? 'pb-0' : ''}`}
											>
												<SubTicketCard
													ticket={child}
													variant='flat'
													readOnly={childrenReadOnly}
													_disableLink={disableLinks}
												/>
											</li>
										))}
									</ul>
	...
	return (
		<Link
			href={`/ticket/${parent.slug}`}
			aria-label={`${parent.title} details`}
			className='block focus:outline-none focus:ring-2 focus:ring-emerald-400 rounded-none'
		>
			{body}
		</Link>
	);
