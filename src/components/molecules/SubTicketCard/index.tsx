...
interface SubTicketCardProps {
	...
	variant?: 'card' | 'flat';
	readOnly?: boolean;
	_disableLink?: boolean;
}

export default function SubTicketCard({
	ticket,
	compact = true,
	variant = 'card',
	readOnly = false,
	...
	const _onToggleSub = (e: React.MouseEvent) => {
		if (readOnly) return;
		e.preventDefault();
		e.stopPropagation();
		setDone(v => !v);
		...
		try {
			...
			if (!response.ok) {
				...
				const revertedTodos = todos.map(t =>
					t.id === id ? { ...t, done: currentTodo.done } : t
				);
	...
	const TitleRow = (
		<div className='min-w-0 flex items-center gap-2'>
			<p
				className={`${compact ? 'text-[12px]' : 'text-[13px]'} font-normal text-neutral-900 truncate`}
			>
				{ticket.title}
			</p>
		...
		<div className='mt-0 pl-6 pb-0'>
			<ul className='space-y-0'>
				{todos.map((t, index) => (
					<li
						key={`${t.id}-${index}`}
						className={index === todos.length - 1 ? 'border-b-0' : ''}
					>
						<div
						...
						>
							<div className='flex items-center gap-2 min-w-0'>
								<StatusDot
									completed={readOnly ? true : !!t.done}
									_variant='todo'
									onClick={readOnly ? undefined : onToggleTodo(t.id)}
									disabled={readOnly}
	...
	const body = (
		<Card
			className={`w-full max-w-full ${compact ? 'pr-1.5 pl-3 py-1.5' : 'p-3'}`}
		>
